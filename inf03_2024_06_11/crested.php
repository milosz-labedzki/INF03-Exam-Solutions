<?php function skrypt1(){
    ...
    $zapytanie = "SELECT DISTINCT data_ur,miot,rasa FROM `swinki`,rasy WHERE swinki.rasy_id=rasy.id AND rasy.id=7;";
    $wynik = mysqli_query($conn,$zapytanie);
    while($wiersz = mysqli_fetch_row($wynik)){
    ...
    $zapytanie = "SELECT imie,cena,opis FROM `swinki` WHERE rasy_id =7;";
    $wynik=mysqli_query($conn,$zapytanie);
    while($wiersz=mysqli_fetch_row($wynik)){
